...
function setNodesAndEdges(pramaNodes,pramaLinks){
    ...
    for(var i=0;i<pramaNodes.length;i++){
        ...
        var text = function () {
            ...
            //  text.setAttribute("class","node-txt");
            // var tspan = document.createElement('tspan');
            // var img = document.createElement('img');
            // img.setAttribute("src",pramaNodes[i].symbol);
            // img.setAttribute("width",pramaNodes[i].width);
            // img.setAttribute("height",pramaNodes[i].height);//字体为宋体
            // tspan.appendChild(document.createTextNode(pramaNodes[i].name));//文本内容"你好"
            //
            // tspan.style.setProperty("font-family","Microsoft Yahei");//字体为宋体
            // tspan.style.setProperty("font-size","15");
            // tspan.style.setProperty("color","#333");
            // tspan.style.setProperty("display",display);
            // text.appendChild(tspan);
            //
            // text.appendChild(img);//文本内容"你好"
            ...
            var table = document.createElement("table"),
                ...
                tr1= d3.select(table).append("tr"),
                tr2= d3.select(table).append("tr"),
                td1 = tr1.append("td"),
                td2 = tr2.append("td");
                td1.text(pramaNodes[i].name);
                img.setAttribute("src",pramaNodes[i].symbol);
                img.setAttribute("width",pramaNodes[i].width);
                img.setAttribute("height",pramaNodes[i].height);//字体为宋体
                td2.html ("<img alt='' src="+pramaNodes[i].symbol+" width="+pramaNodes[i].width+" height="+pramaNodes[i].height+"/>") ;
            return table;


            //return pramaNodes[i].name;
        }




        var obj = {data:pramaNodes[i],label: text(),index:i};
...
function toggleLineToGreen(linkLine, links, isHover) {
    if (isHover) {
        // 加重连线样式
        linkLine
            .filter(function(link){ return isInLink(links, link)})
            .style('opacity', 1)
